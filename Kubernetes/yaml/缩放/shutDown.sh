...
kubectl scale --replicas=0 deployment/gts-article-search  -n spring-app 
gts_info_end=`date`
echo "scale pod gts-article-search.yaml replicas=0 end : $gts_info_end " >> /root/yaml/gangtiseClusterScale.txt

#spring-14 #spring-14 gangtise-user-web
kubectl scale --replicas=0 deployment/gangtise-user-web -n spring-app 
user-web_end=`date`
echo "scale pod gangtise-user-web.yaml replicas=0 end : $user-web_end " >> /root/yaml/gangtiseClusterScale.txt

#spring-15 gangtise-maxwell-web
kubectl scale --replicas=0 deployment/gangtise-maxwell-web -n spring-app 
gts_info_end=`date`
echo "scale pod gangtise-maxwell-web replicas=0 end : $gts_info_end " >> /root/yaml/gangtiseClusterScale.txt

#spring-16 gts-authority-service 
kubectl scale --replicas=0 deployment/gts-authority-service -n spring-app 
gts_info_end=`date`
echo "scale pod gts-authority-service replicas=0 end : $gts_info_end " >> /root/yaml/gangtiseClusterScale.txt
...
az aks scale --resource-group gangtise-k8sGroup --name gangtiseCluster --node-count 2 --nodepool-name agentpool  
node_end=`date`
